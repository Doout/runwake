...
Runwake is a lightweight, read-only viewer for Docker and Kubernetes workloads. It brings live logs, exact runtime events, and current CPU and memory usage into one interface so operators and developers can quickly understand what is happening now.
...
- Does not modify, restart, scale, or delete application workloads.
...
- A Docker socket remains a privileged host capability even though Runwake performs read-oriented operations.
...
4. Preserve read-only intent and communicate infrastructure trust boundaries honestly.
